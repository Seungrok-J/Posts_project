...
const BoardDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchBoardDetail = async (id: string | undefined) => {
            if (id) {
                try {
                    const response = await axios.get(`http://localhost:8080/api/board/detail/${id}`);
    ...
    const handleEdit = () => {
        navigate(`/board/update/${board.id}`); // 수정 페이지로 이동
    };

    const handleDelete = async () => {
        if (window.confirm('정말로 이 게시글을 삭제하시겠습니까?')) {
            try {
                await axios.delete(`http://localhost:8080/api/board/delete/${board.id}`);
                alert('게시글이 삭제되었습니다.');
                navigate('/board'); // 게시글 목록으로 이동
            } catch (error) {
